...
function template(template, character, message) {
  ...
  for (letter of template) {
    if (letter === character && message[counter]) {
        templateModified.push(message[counter]);
        counter++;
    } else {
        templateModified.push(letter);
    }
  }

  return templateModified.join('');
}
