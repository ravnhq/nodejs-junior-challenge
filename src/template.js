 ...
 *
 * All the characters in the template that match the character (second parameter) that is passed
 * to the function must be replaced
 ...
 *
 * Consider validating that the template includes the character and the third parameter is not
 * empty.In the case that one of these conditions is not met, the method must return the
 * message "Invalid parameters"
 *
 * @param {string} templateString - Template to be used
...
function template(templateString, character, message) {
  let templatei = 0;
  let messagei = 0;
  let result = '';
